...
uint8_t broadcastAddress[2][6] = {{0x84, 0x0D, 0x8E, 0x85, 0xF8, 0x1D}, {0x84, 0xCC, 0xA8, 0xB1, 0x5B, 0xFB}};
...
void onDataRecieve(uint8_t * mac_addr, uint8_t *incomingData, uint8_t len) {
  ...
  */
  ...
  if (recieveNotification.type == LED_TYPE) {
    if (recieveNotification.id == 1 || recieveNotification.id == 3) {
...
void loop() {
  ...
  if (sendData) {
    ...
    // Send serialData via ESP-NOW
    esp_now_send(broadcastAddress[0], (uint8_t *) &sendNotification1, sizeof(sendNotification1)); // 1st node 0x1D
    esp_now_send(broadcastAddress[1], (uint8_t *) &sendNotification2, sizeof(sendNotification2)); // 2nd node 0xFB
